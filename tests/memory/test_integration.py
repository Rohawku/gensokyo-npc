...
def test_active_memories_are_rendered_verbatim() -> None:
    assert render_recall([_item("来访者给了我 3 个赛钱。")]) == ["来访者给了我 3 个赛钱。"]
...
def test_compressed_memories_become_one_vague_line_per_kind() -> None:
    ...
    items = [
        _item("来访者说：「一」", Tier.COMPRESSED),
        _item("来访者说：「二」", Tier.COMPRESSED),
        _item("来访者给了我 1 个赛钱。", Tier.COMPRESSED, kind="player_gave_item"),
    ]
...
def test_vivid_memories_come_before_vague_ones() -> None:
    ...
    具体的事。"""
    lines = render_recall([_item("模糊的", Tier.COMPRESSED), _item("清楚的")])

    assert lines[0] == "清楚的"


def test_dormant_memories_are_not_rendered_at_all() -> None:
    assert render_recall([_item("495 年前的事", Tier.DORMANT)]) == []
