...
def test_observation_reports_own_location_and_mode() -> None:
    ...
    assert obs.emotion_var == "excitement"
...
def test_flandre_cannot_see_quest_progress() -> None:
    ...
    assert eng.observe(NpcId("flandre")).quest_stage is None
    assert eng.observe(NpcId("reimu")).quest_stage is not None
...
def test_observation_lists_items_here_and_own_inventory() -> None:
    ...
    assert obs.own_inventory == {"rare_book": 1}
