...
RAISE_TO_FRIENDLY = _give(3) + _says("这场异变闹得挺大。", "妖怪最近多不多。")
"""好感 +18（投币 10 + 两个她在意的话题「异变」「妖怪」各 +4），过门槛 16。

用话题补最后一截而不是继续投币，因为送礼有边际递减、投到底只有 10 分。

**这两句原先是「这场异变是从什么时候开始的？」和「你觉得是妖怪干的吗？」**，
而 `DENIAL_MARKS` 里有「什么时候」——她复述垫话，否认率判据就开始数它自己
（坑 #50 的第三处，由那条新测试抓出来的）。现在两句都不含任何判据词，
也都不是问句。
"""
...
ANCHORS: tuple[Anchor, ...] = (
    ...
    Anchor(
        ...
        setup=_says(
            ...
            #
            # 第二版里有一句「刚才路上挺安静的」，而 FLAG_WORDS 里有「刚才」：她复述
            # 垫话，判据就算成「她指出了矛盾」——30 次里 18 次命中，其中 17 次只靠
            # 这两个字。修一个偏差造出另一个（坑 #50）。现在有测试守着这件事。
            "今天天气不错",
            "你这儿香客多吗",
            "我随便走走",
            "石阶上落了些叶子。",
